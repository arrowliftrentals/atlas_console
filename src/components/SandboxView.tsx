...
const SandboxView: React.FC = () => {
  ...
  const loadProposalDetails = async (proposalId: string) => {
    ...
    try {
      ...
      const atlasApiBase = import.meta.env.VITE_ATLAS_API_BASE || "";
                  ...
                  <div className="mt-3 pt-3 border-t border-gray-700 flex items-center justify-between text-gray-400">
                    <div className="flex gap-4">
                      ...
                      {result.resource_usage && (
                        <>
                          <span>Memory: {result.resource_usage.memory_used_mb.toFixed(1)} MB</span>
                          <span>CPU: {result.resource_usage.cpu_percent.toFixed(1)}%</span>
                        </>
                      )}
                    </div>
                  </div>
