...
export default function DependencyMatrix() {
  ...
  const fetchArchitecture = async (signal?: AbortSignal) => {
    try {
      ...
      const response = await fetch('/v1/architecture/graph', { signal });
      if (response.ok) {
  ...
  const fetchHistoricalTelemetry = async (limit?: number, hours?: number, signal?: AbortSignal) => {
    try {
      ...
      const response = await fetch(`/v1/telemetry/flows?limit=${traceLimit}`, { signal });
      if (response.ok) {
        const data = await response.json();
  ...
  const fetchFlows = async (signal?: AbortSignal) => {
    try {
      const response = await fetch('/v1/telemetry/flows', { signal });
      if (response.ok) {
        const data = await response.json();
        const traces = data.traces || [];
