...
export default function DashboardView() {
  ...
  useEffect(() => {
    const fetchArchitecture = async () => {
      try {
        const archRes = await fetch("/v1/architecture/graph");
        if (archRes.ok) {
          ...
          const [bottlenecksRes, criticalRes, hotRes] = await Promise.all([
            fetch('/v1/telemetry/bottlenecks'),
            fetch('/v1/telemetry/critical-paths'),
            fetch('/v1/telemetry/hot-paths'),
          ]);
  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        ...
        try {
          const driftRes = await fetch("/api/analysis/runs");
          if (driftRes.ok) {
            ...
            let issueCount = 0;
            if (latestRun?.id) {
              const issuesRes = await fetch(`/api/analysis/issues/${latestRun.id}`);
              if (issuesRes.ok) {
        ...
        // Fetch recent flows
        const flowsRes = await fetch("/v1/atlas/logs?limit=20");
        if (flowsRes.ok) {
        ...
        const schedulerRes = await fetch("/api/scheduler/stats");
        if (schedulerRes.ok) {
        ...
        const memoryStatsRes = await fetch("/api/memory/stats");
        ...
        const resourcesRes = await fetch("/api/system/resources");
        if (resourcesRes.ok) {
        ...
        const latestAssessRes = await fetch("/v1/meta/dynamic/latest", {
          cache: 'no-cache',
        });
        ...
        const securityRes = await fetch("/v1/security/summary");
        if (securityRes.ok) {
        ...
        const recommendationsRes = await fetch("/v1/recommendations/summary");
...
id: "drift-review",
              content: (
                <DashboardCard
                  title="Drift Review"
                  subtitle="Documentation drift analysis"
                  onClick={() => setActiveView("drift-review")}
                  accentColor="#f59e0b"
                  info={{
                    title: 'Drift Review card',
                    bullets: [
                      'Detects documentation that has drifted from actual code',
                      'Review and approve/reject suggested corrections',
                      'Covers file paths, class names, function names, API endpoints',
                    ],
                  }}
                >
                  <div className="space-y-2">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="text-xl font-bold text-white/90">{stats.driftIssues}</div>
                        <div className="text-[10px] text-white/40 uppercase">Issues Found</div>
                      </div>
                    </div>
                    <div className="text-xs text-white/50">
                      Last scan: {stats.lastDriftScan}
                    </div>
                  </div>
                </DashboardCard>
              ),
                ...
                >
                  <div className="grid grid-cols-2 gap-1.5">
                    {([
                      ...
                      { label: "Drift Review", view: "drift-review" as ActiveView, icon: "📝" },
                    ]).map(({ label, view, icon }) => (
                      <div
                        key={view}
                        role="button"
