...
export default function NeuralArchitecture3DV2({
  ...
  const handleOptimizeNodePositions = useCallback(() => {
    if (nodes.size === 0) {
      console.warn('[V2] No nodes loaded yet. Wait for architecture to load from /v1/architecture/graph');
      return;
    }
  ...
  useEffect(() => {
    ...
    const connect = () => {
      ...
      try {
        const wsUrl = getAtlasWsUrl('/v1/telemetry/stream');
